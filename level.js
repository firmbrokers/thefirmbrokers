...
(function () {
  ...
  // solidity constants and so can never move once deployed. The public cap
  // STACKS for whitelist wallets — the contract allows a listed wallet
  // WL_CAP + PUBLIC_CAP (6) in total — and connected wallets always show the
  // chain's own mintAllowance(), so these two only ever caption the phases.
  const WL_CAP = 3;
  const PUBLIC_CAP = 3;
  ...
  const PRELAUNCH = {
    minted: null, maxSupply: 2500, priceWei: 3500000000000000n,
  ...
  /// a full attaché. Everything a broker wears has to survive being minted 2,500
  ...
  function paintMintWall() {
    ...
    wall.innerHTML = `<b>${open ? "MINTING NOW" : "MINT HERE"}</b>
      <span>2,500 BROKERS</span><span>0.0035 ETH EACH</span>
  ...
  function currentZone() {
    ...
    const inside = ZONES.find((z) => state.x >= z.x && state.x < z.x + z.w);
  ...
  function openSplitEditor(b, pop, row) {
    ...
    const current = b.split.length ? b.split : [{ idx: usdgIdx(), bps: 10000 }];
  ...
  function docsCards(host, bare) {
    ...
    const items = [
      ["The brokers", "2,500 pixel brokers. " + fmtEth((state.stats || PRELAUNCH).priceWei) + " ETH each. Mint one and you meet him right away — his picture is yours the second the mint goes through. And the collection only ever gets smaller."],
