...
(function () {
  ...
  /// THE MORNING CALL's card, the prediction-market shape (a holder's mock,
  /// 2026-09-15: "what we have now is too chaotic"): the question, two odds
  /// buttons, the pot, the link — nothing else. The caller's own side wears the
  /// gold frame. d: { symbol, dateLong, lock, bell, pays: {up, down} | null,
  /// sides: {up, down, upIn, downIn} | null, calls, pot, side, stake, code, link }
  function drawCall(canvas, d) {
    canvas.width = W; canvas.height = H;
    const x = canvas.getContext("2d");
    const sym = d.symbol || "$9TO5";
    const sides = d.sides || null, pays = d.pays || null;
    const anyIn = !!sides && (sides.upIn || sides.downIn);
    const sizes = {};
    backdrop(x);
    // the head: the mark, the house line, the question, the day and its times
    drawMark(x, d, 48, 48, 88);
    fit(x, `${d.branch || "THE MORNING CALL"} · ${d.house || "FIRM BROKERS"}`.toUpperCase(), 160, 78, { color: C.gold, from: 16, to: 16, max: 990 });
    sizes.q = fit(x, `${sym} UP OR DOWN?`, 160, 128, { color: C.cream, from: 40, to: 28, max: 990 });
    fit(x, [d.dateLong || d.date || "", `call it before ${d.lock || "9:30 AM"} NY`, `decided at the ${d.bell || "4:00 PM"} close`].filter(Boolean).join(" · "), 160, 166, { font: FB, color: C.green, from: 30, to: 24, max: 990 });
    // the two buttons: what each side pays; with one side empty, what is on it
    const BY = 214, BH = 190, BW = 512;
    const label = (side) => {
      if (pays) return `${side}  ${side === "UP" ? pays.up : pays.down}`;
      if (anyIn) { const inn = side === "UP" ? sides.upIn : sides.downIn; return inn ? `${side} · ${side === "UP" ? sides.up : sides.down} in` : `${side} · empty`; }
      return side;
    };
    // one size for both labels (the longer one decides), so the pair reads as a pair
    const measure = (t) => { let size = pays ? 56 : 40; for (; size > 24; size -= 2) { x.font = size + "px " + FD; if (x.measureText(t).width <= BW - 48) break; } return size; };
    const labelPx = Math.min(measure(label("UP")), measure(label("DOWN")));
    const button = (side, bx) => {
      const mine = d.side === side;
      const up = side === "UP";
      frame(x, bx, BY, BW, BH, mine);
      x.fillStyle = up ? "#0d3b20" : "#3d0f12"; x.fillRect(bx, BY, BW, BH);
      x.fillStyle = up ? "rgba(182,255,207,0.08)" : "rgba(255,138,122,0.08)"; x.fillRect(bx, BY, BW, 6);
      const col = up ? C.bright : "#ff8a7a";
      x.save(); x.shadowColor = col; x.shadowBlur = 24;
      const lifted = mine || (anyIn && !pays && !(up ? sides.upIn : sides.downIn));
      sizes[up ? "up" : "down"] = fit(x, label(side), bx + BW / 2, BY + BH / 2 + (lifted ? 8 : 20), { color: col, from: labelPx, to: labelPx, max: BW - 48, align: "center" });
      x.restore();
      if (mine) fit(x, `YOUR CALL${d.stake ? ` · ${d.stake} $9TO5` : ""}`, bx + BW / 2, BY + BH - 30, { color: C.gold, from: 16, to: 16, max: BW - 48, align: "center" });
      // the empty side, once the other has calls: the invitation is the fact
      else if (anyIn && !pays && !(up ? sides.upIn : sides.downIn)) fit(x, `FIRST ${side} CALL SETS THE ODDS`, bx + BW / 2, BY + BH - 30, { color: col, from: 16, to: 16, max: BW - 48, align: "center" });
    };
    button("UP", 62); button("DOWN", 626);
    // the foot: the pot at the left, the sender's code and link at the right
    const n = Number(d.calls || 0);
    const potLine = n > 0 ? `${d.pot || "0"} $9TO5 in the pot · ${n} call${n === 1 ? "" : "s"}` : "no calls yet · the pot is open";
    fit(x, potLine, 48, 496, { font: FB, color: C.cream, from: 40, to: 28, max: 620 });
    const sub = d.side ? `${d.code ? d.code + " called" : "called"} ${d.side}${d.stake ? ` with ${d.stake} $9TO5` : ""} · winners split the losing side`
      : anyIn && !pays ? `the first ${sides.upIn ? "DOWN" : "UP"} call sets the odds · winners split the losing side`
      : n > 0 ? "winners split the losing side · settled on chainlink's feed" : "the first call sets the odds · winners split the losing side";
    fit(x, sub, 48, 538, { font: FB, color: C.green, from: 28, to: 22, max: 620 });
    if (d.code) {
      x.save(); x.shadowColor = C.gold; x.shadowBlur = 14;
      sizes.code = fit(x, `SENT BY ${String(d.code).toUpperCase()}`, 1152, 494, { color: C.gold, from: 18, to: 16, max: 380, align: "right" });
      x.restore();
    }
    if (d.link) fit(x, String(d.link).replace(/^https?:\/\//, ""), 1152, 534, { font: FB, color: C.grey, from: 28, to: 22, max: 380, align: "right" });
    scanlines(x);
    return sizes;
  }
  ...
  const cardBlob = () => new Promise((res) => {
    ...
    const draw = state.kind === "winner" ? drawWinner : state.kind === "call" ? drawCall : drawCard;
    draw(cv, state);
  ...
  async function open(d) {
    ...
    modal.innerHTML = `<div class="pc-box"><div class="pc-head"><span class="lab">YOUR CARD</span><button class="chip pc-x" type="button">CLOSE</button></div>
      ${winner || call ? "" : `<div class="pc-row"><span class="dim">your X handle</span><input type="text" class="pc-handle" placeholder="optional · puts your picture on it" value="${esc(state.handle)}" autocapitalize="off" spellcheck="false"></div>`}
    ...
    paint();
    if (state.handle && !winner && !call) loadPfp(state.handle).then((img) => { state.pfp = img; paint(); });
