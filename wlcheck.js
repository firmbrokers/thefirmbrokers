...
(function () {
  ...
  async function checkChain(addr) {
    ...
    if (!open) {
      return listed
        ? ["on", "YOU ARE IN", "You mint first: 3 early, then 3 more in public. 6 total.", open]
    ...
    return [left > 0 ? "on" : "hold", "DOORS OPEN TO EVERYONE",
      (listed ? "A listed wallet: 6 in total. " : "") +
  ...
  async function checkList(addr, wl) {
    ...
    if (hit) {
      return ["on", "YOU ARE IN",
        "You made the list. You mint first: 3 early, then 3 more in public. 6 total."];
    }
