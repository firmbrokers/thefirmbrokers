...
window.FIRM_CFG = {
  ...
  // `mark` is the community's 48x48 pixel mark under /art/marks/ (absolute: it must resolve from /pool/<slug>; a missing
  // file falls back to a drawn coin). Empty list = the building stays the
  // Auction House and no branch UI is built anywhere.
  // `period` (seconds, optional; absent = 86400, a daily bell) is the branch's
  // cadence: every surface reads its words from branches.js periodWords ("every
  // 4 hours", "this round", "the next pot") and the pool page shows the 3-player
  // rule for a faster branch. THE PUNCH CLOCK (PunchClockPool, a bell every four
  // hours, three players at the bell or it moves on) is added at its deploy as:
  //   { slug: "clock", name: "THE PUNCH CLOCK", short: "PUNCH CLOCK", symbol: "$9TO5", token: "0x223E93B1beD7de244445dB2dea4c7900e8045Acc", pool: <the deploy's address>, block: <its block>, page: "/pool/clock", boost: true, mark: "/art/marks/clock.png", period: 14400 },
  // then `node script/branch-pages.mjs` for site/pool/clock.html. Two branches
  // in one symbol are told apart by `short` on the tabs, the wall and the board.
  branches: [
    ...
    { slug: "frong", name: "THE FRONG POND",  short: "FRONG POND",  symbol: "$FRONG", token: "0x6245e67affA44a23077f0Ea7f981a8DC743a0c47", pool: "0x16f2D383e1C09d48568c882CD1B323B99f592842", block: 55666061, page: "/pool/frong", boost: false, mark: "/art/marks/frong.png" }, // its own page here (site/pool/frong.html, generated by script/branch-pages.mjs); frong.io/pond stays that community's own door
    { slug: "cashcat", name: "THE CASHCAT DESK", short: "CASHCAT", symbol: "$CASHCAT", token: "0x020bfC650A365f8BB26819deAAbF3E21291018b4", pool: "0x2F75E3bBbAc257937Dd941e508427047963542Ab", block: 57844318, page: "/pool/cashcat", boost: true, mark: "/art/marks/cashcat.png" }, // THE CASHCAT DESK, deployed 2026-09-08 (block 57844318): brokers boost like HQ; the house cut is split 50/50 with the Cash Cat side off-chain
    { slug: "clock", name: "THE PUNCH CLOCK", short: "PUNCH CLOCK", symbol: "$9TO5", token: "0x223E93B1beD7de244445dB2dea4c7900e8045Acc", pool: "0x8a573d9541AF157e65F01BD9b157D77980f73eE0", block: 61279758, page: "/pool/clock", boost: true, mark: "/art/marks/clock.png", period: 14400 }, // THE PUNCH CLOCK (PunchClockPool, deployed 2026-09-12 17:19Z, SeedBox 0xF92dB095be88a4F62f8678B4A47BaEF22626872E): a bell every four hours on the UTC grid, three players at the bell or it moves on (twice at most); 2,500 min, brokers boost like HQ
  ],
  ...
  call: "0xcc2eE1e12B5e530Bbbfd30cc9203F0248f4E7eb8", // MorningCall, deployed 2026-09-12 17:07Z from the admin key (12 txs, block 61273334); first round Mon 2026-09-14 NVDA
  callBlock: 61273334,
